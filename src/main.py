...
class NameChangeBot:
    def __init__(self):
        ...
        # Command lock to prevent duplicate processing
        self.command_locks = {}
    ...
    async def list_scam_command(self, event):
        ...
        try:
            # Create a unique lock key for this command and user
            lock_key = f"listscam_{event.sender_id}"
            
            # Check if command is already being processed
            if lock_key in self.command_locks:
                return
            
            # Set the lock
            self.command_locks[lock_key] = True
            ...
            try:
                ...
                self.command_locks.pop(lock_key, None)
            ...
            # Make sure to remove the lock even if there's an error
import asyncio
import logging
from telethon import TelegramClient, events
from telethon.tl.types import User, UpdateUser, UpdateUserName, PeerChannel, UpdateChannelParticipant, UpdateUserStatus
